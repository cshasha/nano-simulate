...
h0 = 0.02  #applied field amp (T/mu0). 10 Oe = 0.001 T
...
K = 10000.   #anisotropy constant (J/m^3). 1 J/m^3 = 10 erg/cm^3
sign1 = 1  #+1 or -1
K2 = 2000.
sign2 = -1   #+1 or -1
#sd = 3000.   #standard deviation of K
sd = 1.
k_sigma = np.sqrt(np.log(1 + sd**2*K**(-2)))   #variance for k values
...
Axesx = np.zeros((I,3,N))   #initialize anisotropy axes
Axesy = np.zeros((I,3,N))   #initialize anisotropy axes
...
def initialize():
	...
	if aligned == "yes":
		...
		theta_x = np.arctan(-np.cos(An_theta)/np.sin(An_theta))
		nx[:,0] = np.sin(theta_x[:])*np.cos(An_phi[:])
		nx[:,1] = np.sin(theta_x[:])*np.sin(An_phi[:])
		nx[:,2] = np.cos(theta_x[:])
		ny = np.cross(An,nx,axis=1)
...
def thermalize(temp):
	#Ms = M0*(1 - b*temp**a)
	global nx, ny
	betas = 2*Ms**(-1)*k_values 
	betas2 = 2*Ms**(-1)*k_values2
	...
	for n in range(int(N/5)):
		...
		if shape == "u":
			...
			H = betas[:,None]*np.sum(M[:,:,n]*Axes[:,:,n],axis=1)[:,None]*Axes[:,:,n]
		else:
			mx = np.sum(M[:,:,n]*nx,axis=1)[:,None]
			my = np.sum(M[:,:,n]*ny,axis=1)[:,None]
			mz = np.sum(M[:,:,n]*Axes[:,:,n],axis=1)[:,None]
			if brownian == "on":
				Torque = 2*(k_values*volumes)[:,None]*(my**2 + mx**2)*np.sum(M[:,:,n]*Axes[:,:,n],axis=1)[:,None]*np.cross(Axes[:,:,n],M[:,:,n]) + 2*(k_values2*volumes)[:,None]*my**2*mx**2*np.sum(M[:,:,n]*Axes[:,:,n],axis=1)[:,None]*np.cross(Axes[:,:,n],M[:,:,n])

			H = -betas[:,None]*(mx**2*mz*Axes[:,:,n] + my**2*mz*Axes[:,:,n] + mx**2*my*ny + mz**2*my*ny + my**2*mx*nx + mz**2*mx*nx) - betas2[:,None]*(mx**2*my**2*mz*Axes[:,:,n] + mx**2*mz**2*my*ny + my**2*mz**2*mx*nx)
		...
		if brownian == "on":
			...
			hbar = betas[:,None]*np.sum(mb*nb,axis=1)[:,None]*nb
			if interactions == "on":
				hbar += H_dip
			if shape == "u":
			...
			hbar = betas[:,None]*np.sum(mb*An,axis=1)[:,None]*An
			Axes[:,:,n+1] = An
			if interactions == "on":
				hbar += H_dip
...
def runMC(temp, bias):
	...
	for n in range(N-1):
		...
		if interactions == "on":
			...
			y[:,:,1] = z
			y[:,:,2] = z
			#added mu0 (for mu0/4pi), changed 24 to 8
			Q = ma.masked_array((mu0*diam1**3/8.*Ms*(3*np.sum(RR*mGhost,axis=2)[:,:,None]*RR/R[:,:,None]**2 - mGhost))/R[:,:,None]**3, mask=y, fill_value = 0)
			H_dip = np.sum(Q.filled(0)[0:I,:,:],axis=1)
			H += H_dip
		...
		if brownian == "on":
			...
			hbar = betas[:,None]*np.sum(mb*Axes[:,:,n],axis=1)[:,None]*Axes[:,:,n]
			hbar[:,1] += H_app_y[n+1]
			hbar[:,2] += H_app_z[n+1] + H_bias[n+1]  #added bias here
			Axes[:,:,n+1] = An
			if interactions == "on":
